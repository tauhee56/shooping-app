...
const mockDB = {
    ...
    findProduct: (query) => products.find(p => {
        if (query._id)
            return p._id === query._id;
        if (query.store)
            return p.store === query.store;
        return false;
    }),
    ...
    findOrder: (query) => orders.find(o => {
        if (query._id)
            return o._id === query._id;
        if (query.buyer)
            return o.buyer === query.buyer;
        return false;
    }),
    ...
    getMessagesBetween: (userId, otherId) => messages.filter(m => (m.sender === userId && m.receiver === otherId) ||
        (m.sender === otherId && m.receiver === userId)).sort((a, b) => {
