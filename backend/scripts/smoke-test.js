...
(async () => {
  try {
    ...
    // Health
    const health = await axios.get(`${BASE_URL}/health`).then(r => r.data).catch(e => ({ error: e.message }));
    ...
    const api1 = axios.create({ baseURL: BASE_URL, headers: { Authorization: `Bearer ${t1}` } });
    const api2 = axios.create({ baseURL: BASE_URL, headers: { Authorization: `Bearer ${t2}` } });
