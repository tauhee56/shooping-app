...
(async (): Promise<void> => {
  try {
    ...
    // Health
    const health = await axios.get(`${BASE_URL}/health`).then(r => r.data).catch((e: any) => ({ error: e.message }));
    if (health && health.status) log('Health check', true, `${health.status} | DB: ${health.db || 'unknown'}`); else throw new Error('Health failed');

    // Register two users
    const r1 = await axios.post(`${BASE_URL}/auth/register`, u1).then(r => r.data);
    const r2 = await axios.post(`${BASE_URL}/auth/register`, u2).then(r => r.data);
    log('Register users');

    const t1: string = r1.token; const t2: string = r2.token;
    const api1: AxiosInstance = axios.create({ baseURL: BASE_URL, headers: { Authorization: `Bearer ${t1}` } });
    const api2: AxiosInstance = axios.create({ baseURL: BASE_URL, headers: { Authorization: `Bearer ${t2}` } });

    // Login to verify
    const l1 = await axios.post(`${BASE_URL}/auth/login`, { email: u1.email, password: u1.password }).then(r => r.data);
    const l2 = await axios.post(`${BASE_URL}/auth/login`, { email: u2.email, password: u2.password }).then(r => r.data);
    log('Login users');
    ...
    if (!Array.isArray(conv)) throw new Error('Conversations not array');
    ...
    if (!order || !order._id) throw new Error('Order create failed');
    const myOrders = await api2.get('/orders').then(r => r.data);
    if (!Array.isArray(myOrders)) throw new Error('Get orders failed');
